...
public class BSPTree : MonoBehaviour {
	...
	public class BSPTreeNode {
		...
		public bool Split(int minRoomSize, int maxRoomSize) {
			...
			bool splitH;// 방을 나누는 조건을 정해서 나눈다(가로, 세로의 길이에 따라서)
			...
			if (splitH) {
				//나누어진 결과대로 나두되, 범위는 또한 랜덤이 되도록 나눈다.
				int split = Random.Range (minRoomSize, (int)(rect.width - minRoomSize));

				left = new BSPTreeNode (new Rect (rect.x, rect.y, rect.width, split));
				...
				left = new BSPTreeNode (new Rect (rect.x, rect.y, split, rect.height));
				right = new BSPTreeNode (
		...
		public void CreateRoom() {
			...
			if (IAmLeaf()) {
				...
				room = new Rect (rect.x + roomX, rect.y + roomY, roomWidth, roomHeight);//방은 절대적인 좌표에 있도록
				Debug.Log ("Created room " + room + " in sub-dungeon " + debugId + " " + rect);
			}
		}


		public void CreateCorridorBetween(BSPTreeNode left, BSPTreeNode right) {
	...
	public void CreateBSP(BSPTreeNode subDungeon) {
	...
	public void DrawRooms(BSPTreeNode subDungeon) {
		if (subDungeon == null) {
			return;
		}
		if (subDungeon.IAmLeaf()) {
	...
	void DrawCorridors(BSPTreeNode subDungeon) {
		if (subDungeon == null) {
			return;
		}
	...
	void Start () {
		BSPTreeNode root_Tree = new BSPTreeNode (new Rect (0, 0, width, height));
		CreateBSP (root_Tree);
		root_Tree.CreateRoom ();

		boardPositionsFloor = new GameObject[width, height];
		DrawRooms (root_Tree);
		DrawCorridors (root_Tree);
		for(int x = 0; x < width; x++)
        {
			for (int y = 0; y < height; y++)
            {
                ...
                {
					Instantiate(wallTile, new Vector3(x, y, 0f), Quaternion.identity);
                }
            }
        }
	}
